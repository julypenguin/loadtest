...
function loadtest(setting = {}) {
    ...
    return {
        ...
        resIdx,
        cookie,
        setError({ statusCode, res, res_time, cookie }) {
        ...
        test(option = {}, cookie, urlSetting) {
            ...
            return fetch(`${usingApi}${newUrl}`, {
                ...
                body: method.toUpperCase() === 'GET' ? undefined : JSON.stringify(this.body)
            })
                .then(res => {
                ...
                .then(res => {
                    ...
                    if (!!statusCode) {
                        this.setError({ statusCode, res, res_time, cookie })
                    ...
                    if ((typeof failResult === 'string' && res.includes(failResult)) || (typeof succResult === 'string' && !res.includes(succResult))) {
                        this.setError({ statusCode: -1, res, res_time, cookie })
                ...
                .catch(error => {
                    this.resIdx = this.resIdx + 1
                    console.log(`${usingApi}${newUrl}`, error) // 印出哪支 api 發生錯誤以及錯誤原因
                    return {
                        res_time: 0,
                        res: {},
                    }
